...
for i in range(len(test_label)):
    true_class = np.argmax(test_label[i])
    predicted_class = np.argmax(test_predictions[i])
    if true_class != predicted_class:
        misclassified_samples.append((test_image[i], true_class, predicted_class))
        misclassified_indices.append(i)

misclassification_groups = defaultdict(list)

for sample in misclassified_samples:
    true_class, predicted_class = sample[1], sample[2]
    misclassification_groups[(true_class, predicted_class)].append(sample)

def misclassifications(misclassification_group):
    num_samples = len(misclassification_group)
    ...
    for i, (image, true_class, predicted_class) in enumerate(misclassification_group):
        row = i // num_cols
        col = i % num_cols
        ax = axes[row, col]

        ax.imshow(image.squeeze(), cmap='gray')
        ax.set_title(f"True: {true_class}, Predicted: {predicted_class}")
        ax.axis('off')
...
misclassifications(misclassification_groups[(true_class, predicted_class)])
